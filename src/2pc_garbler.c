...
int
garbler_online(char* function_path, int *inputs, int num_garb_inputs, int num_eval_inputs,
           ...
           int num_chained_gcs, int* circuitMapping, int *inputs, unsigned long *ot_time)
{
    ...
    // Loop over input mapping and copy the appropriate labels into
    // garbLabels and evalLabels based on the specification in the input_mapping
    ...
    /* assume eval knows num_garb_inputs */
    if (num_garb_inputs > 0) {
